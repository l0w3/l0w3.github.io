...
Since the frontend is not much the main point of this development, I won't dive deep on it, just note that it holds enough to get a DLL path and a button to trigger the execution.
...
After a successful execution I checked the alerts generated on the timeframe where I executed the tests:
...
For defenders it might be a good idea to create custom rules that detect this type of behavior. Note that on my instance, Elastic has all the rules activated, as well as the Elastic Defend Integration.
