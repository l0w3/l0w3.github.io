...
`madvise` is a syscalll that allows developers to tell it how memory mappings will be used in the near future. We could use  `DONT-USE` to tell it that the memory mapping will not be used and therefore the kernel might free it at some point. A key thing to understand is that, future access to mappings that have been freed my `madvise(DONT-USE)` will result in pointing back to the original physical address of the mapping
...
1. When we first create the mapping and read from the file, the virtual address will point to where the original file is loaded in physical memory

![Screenshot from 2024-12-30 15-35-53](https://github.com/user-attachments/assets/4a6a7e2d-17de-4777-b8c5-06193e571824)

2. When we try to write to it, CoW kicks in, creating a different mapping for the changes to be done.

![Screenshot from 2024-12-30 15-36-23](https://github.com/user-attachments/assets/8ae20899-14be-4eed-99aa-d2b1f1b54503)

3. When `madvise` is executed, the virtual address points back to the original physical address
...
What would happen if after the kernel allocates memory on the COW operation and just before it writes the data, a madvise operation with DONTUSE is performed?
